...
public class FestivalFormController {
    public AnchorPane festivalPane;
    ...
    public void btnSaveOnAction(ActionEvent actionEvent) {
        try {
            ...
            String status=txtSStus.getText();
            ...
            if (isAdded) {
                ...
                new Alert(Alert.AlertType.CONFIRMATION, "festival isn't added").show();
            }
        } catch (Exception e) {
    ...
    public void btnUpdateOnAction(ActionEvent actionEvent) {
        ...
        String location = txtLocation.getText();
        String locationRegex="^[A-Za-z\\s]{1,100}$";
        if (!location.matches(locationRegex)) {
            new Alert(Alert.AlertType.ERROR, "Invalid location format. Please enter a valid location.").show();
            return;
        }
        String aId = String.valueOf(cmbApId.getValue());
        LocalTime time = LocalTime.parse(txtTime.getText());
        LocalDate date = LocalDate.parse((txtDate.getText()));
        String theme = txtTheme.getText();
        String themeRegex="^[A-Za-z\\s]{1,10}$";
        if (!theme.matches(themeRegex)) {
            new Alert(Alert.AlertType.ERROR, "Invalid location theme. Please enter a valid theme.").show();
            return;
        }
        String status=txtSStus.getText();
        ...
        var dto = new EventDesignDto(id, type, location, aId, time, date, theme,status);        try {
            ...
            if (isUpdated) {
                ...
                new Alert(Alert.AlertType.CONFIRMATION, "festival isn't updated").show();
            }
        } catch (Exception e) {
            new Alert(Alert.AlertType.ERROR, e.getMessage()).show();
        }
    ...
    public void btnDeleteOnAction(ActionEvent actionEvent) {
        ...
        try{
            ...
            if(isDeleted){
                ...
                new Alert(Alert.AlertType.CONFIRMATION," festival is not deleted");
            }
        }catch(Exception e){
            new Alert(Alert.AlertType.ERROR,e.getMessage()).show();
        }
    ...
    public void btnBackOnAction(ActionEvent actionEvent) throws IOException {
        Navigation.navigate(Route.EVENT_DESIGN,festivalPane);
    }
    private boolean validateFields() {
      ...
      if (!validate(txtDate.getText(), dateRegex)) {
        ...
        if (!validate(txtSStus.getText(), statusRegex)) {
